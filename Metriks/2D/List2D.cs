...
public class List2D<T> : IList2D<T>, ICollection2D, IReadonlyList2D<T> {
    ...
    /// </exception>
    public void Fill(T item, Point2D offset, Size2D size) 
        => Fill(item, offset.X, size.X, offset.Y, size.Y);
    ...
    /// Thrown when the specified region exceeds the bounds of the list or one of the size parameters is negative.
    /// </exception>
    public void Fill(Func<T> factory, Point2D offset, Size2D size) 
        => Fill(factory, offset.X, size.X, offset.Y, size.Y);
