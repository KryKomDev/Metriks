...
public readonly ref struct ReadOnlySpan2D<T> {
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="ReadOnlySpan2D{T}" /> struct wrapping a flat read-only span with given
    ///     dimensions.
    /// </summary>
    ...
    public ReadOnlySpan2D<T> Slice(int x, int y, int xSize, int ySize) {
        ...
        return new ReadOnlySpan2D<T>(_span.Slice(offset, length), xSize, ySize, Stride);
    }
