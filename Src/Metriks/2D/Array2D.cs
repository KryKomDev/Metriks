...
public static class Array2D {
    ...
    public static void Copy<T>(
        ...
        if (xToCopy <= 0 || yToCopy <= 0)
            return;

        var srcCols = source.GetLength(1);
        var dstCols = dest.GetLength(1);
    ...
    /// <summary>
    ///     Copies a rectangular region of data from one two-dimensional array to another, using structured definitions for
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void Copy<T>(T[,] source, T[,] dest, Point2D offset) => Copy(source, Point2D.Zero, dest, offset, dest.Size);
    ...
    public static void Fill<T>(T[,] array, T item, int xStart, int yStart, int xCount, int yCount) {
        ...
        for (var i = 0; i < xCount; i++) {
            var offset = (xStart + i) * cols + yStart;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void Fill<T>(T[,] array, T item, Point2D start, Size2D size) => Fill(array, item, start.X, start.Y, size.X, size.Y);
    ...
    /// <param name="size">The size of the rectangular region within the array.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void Fill<T>(T[,] array, Func<T> itemFactory, Point2D start, Size2D size) => Fill(array, itemFactory, start.X, start.Y, size.X, size.Y);
    ...
    public static void Clear<T>(T[,] array, int xOffset, int yOffset, int xCount, int yCount) {
        ...
        for (var i = 0; i < xCount; i++) {
            var index = (xOffset + i) * cols + yOffset;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void Clear<T>(T[,] array, Point2D offset, Size2D size) => Clear(array, offset.X, offset.Y, size.X, size.Y);
