...
public readonly ref struct Span2D<T> {
    ...
    public Span2D<T> Slice(int x, int y, int xSize, int ySize) {
        ...
        return new Span2D<T>(_span.Slice(offset, length), xSize, ySize, _stride);
    }
    ...
    public void CopyTo(Span2D<T> destination) {
        if (_xSize != destination._xSize || _ySize != destination._ySize) {
    ...
    public static implicit operator ReadOnlySpan2D<T>(Span2D<T> span) => 
        new(span._span, span._xSize, span._ySize, span._stride);
