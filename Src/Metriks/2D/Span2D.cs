...
/// <summary>
///     Represents a two-dimensional view of a contiguous or strided memory region.
/// </summary>
...
public readonly ref struct Span2D<T> {
    ...
    /// </summary>
    public Size2D Size => new(XSize, YSize);
    ...
    /// </summary>
    public bool IsEmpty => XSize == 0 || YSize == 0;
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="Span2D{T}" /> struct wrapping a two-dimensional array.
    /// </summary>
    ...
    public Span2D(T[,] array) {
        ...
        _span = XSize == 0 || YSize == 0
            ? Span<T>.Empty
            : MemoryMarshal.CreateSpan(ref array[0, 0], array.Length);
    }

    /// <summary>
    ///     Initializes a new instance of the <see cref="Span2D{T}" /> struct wrapping a flat span with given dimensions.
    /// </summary>
    ...
    public Span2D(Span<T> span, int xSize, int ySize) {
        if (xSize < 0)
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="Span2D{T}" /> struct wrapping a strided memory region.
    /// </summary>
    /// <param name="span">The memory span starting at the top-left element.</param>
    /// <param name="xSize">The size along the X-axis.</param>
    /// <param name="ySize">The size along the Y-axis.</param>
    /// <param name="stride">The row-to-row stride.</param>
    public Span2D(Span<T> span, int xSize, int ySize, int stride) {
        if (xSize < 0)
            throw new ArgumentOutOfRangeException(nameof(xSize));
        ...
        var requiredLength = xSize == 0 || ySize == 0 ? 0 : (xSize - 1) * stride + ySize;

        if (requiredLength > span.Length)
            throw new ArgumentException("Span length is less than the specified dimensions and stride.");
    ...
    /// <summary>
    ///     Gets a reference to the element at the specified 2D indices.
    /// </summary>
    ...
    public ref T this[int x, int y] {
        ...
        get {
            if ((uint)x >= (uint)XSize || (uint)y >= (uint)YSize)
                throw new IndexOutOfRangeException();

            return ref _span[x * Stride + y];
        }
    }
    ...
    /// <summary>
    ///     Slices the two-dimensional span.
    /// </summary>
    ...
    public Span2D<T> Slice(int x, int y, int xSize, int ySize) {
        if (x < 0 || y < 0 || xSize < 0 || ySize < 0)
            throw new ArgumentOutOfRangeException();

        if (x + xSize > XSize || y + ySize > YSize)
            throw new ArgumentOutOfRangeException();

        if (xSize == 0 || ySize == 0)
            return default;

        var offset = x           * Stride + y;
        var length = (xSize - 1) * Stride + ySize;

        return new Span2D<T>(_span.Slice(offset, length), xSize, ySize, Stride);
    }
    ...
    /// <returns>A new <see cref="Span2D{T}" /> representing the sliced region.</returns>
    public Span2D<T> Slice(Point2D offset, Size2D size) => Slice(offset.X, offset.Y, size.X, size.Y);
    ...
    /// <returns>A span containing the elements of the row.</returns>
    public Span<T> GetRow(int x) {
        if ((uint)x >= (uint)XSize)
            throw new ArgumentOutOfRangeException(nameof(x));

        return _span.Slice(x * Stride, YSize);
    }
    ...
    public void CopyTo(Span2D<T> destination) {
        if (XSize != destination.XSize || YSize != destination.YSize)
    ...
    /// <summary>
    ///     Implicitly converts a two-dimensional array to a <see cref="Span2D{T}" />.
    /// </summary>
    public static implicit operator Span2D<T>(T[,] array) => new(array);

    /// <summary>
    ///     Implicitly converts a <see cref="Span2D{T}" /> to a <see cref="ReadOnlySpan2D{T}" />.
    /// </summary>
    public static implicit operator ReadOnlySpan2D<T>(Span2D<T> span) => new(span._span, span.XSize, span.YSize, span.Stride);
    ...
    /// <summary>
    ///     Enumerates elements of a <see cref="Span2D{T}" />.
    /// </summary>
    public ref struct Enumerator {
        private readonly Span2D<T> _span;
        private          int       _x;
        private          int       _y;
