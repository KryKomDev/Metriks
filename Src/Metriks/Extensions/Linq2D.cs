...
public static class Linq2D {
    ...
    extension<T>(IEnumerable2D<T> list) {
        ...
        /// <returns>
        ///     An <see cref="IEnumerable{T}" /> whose elements are the result of invoking the projection
        ///     function on each sub-collection.
        /// </returns>
    ...
    extension<T>(Space2D<T> space2D) {
        ...
        /// </returns>
        public bool AllAtX(int x, Predicate<T> predicate) {
            for (var y = 0; y < space2D.YSize; y++)
                if (!predicate(space2D.GetUnsafe(x + space2D.XOriginOffset, y)))
                    return false;
        ...
        /// </exception>
        public bool AllAtY(int y, Predicate<T> predicate) {
            for (var x = 0; x < space2D.XSize; x++)
                if (!predicate(space2D.GetUnsafe(x, y + space2D.YOriginOffset)))
                    return false;
