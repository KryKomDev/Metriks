...
public readonly ref struct ReadOnlySpan3D<T> {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ReadOnlySpan3D{T}"/> struct wrapping a flat span with given dimensions.
    /// </summary>
    ...
    public ReadOnlySpan3D<T> Slice(int x, int y, int z, int xSize, int ySize, int zSize) {
        ...
        return new ReadOnlySpan3D<T>(_span.Slice(offset, length), xSize, ySize, zSize, _strideX, _strideY);
    }
    ...
    public ReadOnlySpan2D<T> GetPlaneAtX(int x) {
        ...
        return new ReadOnlySpan2D<T>(_span.Slice(offset, length), _ySize, _zSize, _strideY);
    }
    ...
    public void CopyTo(Span3D<T> destination) {
        if (_xSize != destination.XSize || _ySize != destination.YSize || _zSize != destination.ZSize) {
