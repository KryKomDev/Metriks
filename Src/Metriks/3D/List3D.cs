...
public class List3D<T> : IList3D<T>, ICollection3D, IReadOnlyList3D<T> {
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="List3D{T}" /> class that contains elements copied
    ///     from the specified <see cref="List3D{T}" />.
    /// </summary>
    /// <param name="other">The <see cref="List3D{T}" /> whose elements are copied to the new list.</param>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="other" /> is null.</exception>
    public List3D(List3D<T> other) {
        ...
        var totalCapacity = XCapacity * YCapacity * ZCapacity;
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="List3D{T}" /> class that contains elements copied
    ///     from the specified three-dimensional list.
    /// </summary>
    /// <param name="other">The three-dimensional list whose elements are copied to the new list.</param>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="other" /> is null.</exception>
    public List3D(IReadOnlyList3D<T> other) {
        ArgumentNullException.ThrowIfNull(other);

        if (other is List3D<T> otherList) {
        ...
        var count = XCapacity * YCapacity * ZCapacity;
        if (count == 0) {
            _items = Array.Empty<T>();
            return;
        }
        ...
        for (var x = 0; x < XSize; x++) {
            for (var y = 0; y < YSize; y++) {
                for (var z = 0; z < ZSize; z++) {
                    _items[GetOffset(x, y, z)] = other[x, y, z];
                }
            }
        }
    }
    ...
    /// <summary>
    ///     Creates a shallow copy of the current <see cref="List3D{T}" />.
    /// </summary>
    /// <returns>A new <see cref="List3D{T}" /> containing a copy of the elements.</returns>
    [Pure]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public List3D<T> Clone() => new(this);
    ...
    /// <summary>Copies elements to the specified multidimensional array starting at (0, 0, 0).</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void CopyTo(T[,,] array) => CopyTo(array, Point3D.Zero);

    /// <summary>
    ///     Copies the elements of the <see cref="List3D{T}" /> to the specified destination <see cref="List3D{T}" />.
    ///     If the destination list is empty, it is resized to match this list; otherwise, elements are copied starting at (0, 0, 0).
    /// </summary>
    ...
    public void CopyTo(List3D<T> destination) {
    ...
    /// <summary>
    ///     Copies all elements of the <see cref="List3D{T}" /> to the specified destination <see cref="List3D{T}" />
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="destination" /> is null.</exception>
    public void CopyTo(List3D<T> destination, Point3D destinationIndex) {
        ArgumentNullException.ThrowIfNull(destination);
    ...
    /// <summary>
    ///     Copies a rectangular region of elements from this list to the specified destination <see cref="List3D{T}" />.
    /// </summary>
    /// <param name="destination">The destination list.</param>
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="destination" /> is null.</exception>
    public void CopyTo(List3D<T> destination, Point3D sourceOffset, Point3D destinationOffset, Size3D size) {
        ArgumentNullException.ThrowIfNull(destination);
    ...
    /// <summary>
    ///     Copies all elements from the specified <see cref="List3D{T}" /> into this list,
    ...
    public void CopyFrom(List3D<T> source) {
        ...
        if (YCapacity == source.YCapacity && ZCapacity == source.ZCapacity) {
            source._items.AsSpan(0, XSize * YCapacity * ZCapacity).CopyTo(_items);
        }
        else {
            for (var x = 0; x < XSize; x++) {
                for (var y = 0; y < YSize; y++) {
                    source._items.AsSpan(source.GetOffset(x, y, 0), ZSize)
                        .CopyTo(_items.AsSpan(GetOffset(x, y, 0), ZSize));
                }
            }
        }
    }
    ...
    /// <summary>
    ///     Copies all elements from the specified three-dimensional list into this list,
    ///     resizing this list to match the dimensions of the source list.
    /// </summary>
    /// <param name="source">The source list to copy from.</param>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="source" /> is null.</exception>
    public void CopyFrom(IReadOnlyList3D<T> source) {
        ...
        for (var x = 0; x < XSize; x++) {
            for (var y = 0; y < YSize; y++) {
                for (var z = 0; z < ZSize; z++) {
                    _items[GetOffset(x, y, z)] = source[x, y, z];
                }
            }
        }
    }
    ...
    /// <summary>
    ///     Copies all elements from the specified <see cref="List3D{T}" /> into this list starting at
    ...
    public void CopyFrom(List3D<T> source, Point3D destinationOffset) {
        ArgumentNullException.ThrowIfNull(source);

        CopyFrom(source, Point3D.Zero, destinationOffset, source.Size);
    }

    /// <summary>
    ///     Copies all elements from the specified three-dimensional list into this list starting at
    ///     the specified destination offset without modifying this list's dimensions.
    /// </summary>
    /// <param name="source">The source list to copy from.</param>
    /// <param name="destinationOffset">The zero-based coordinate in this list at which copying begins.</param>
    ...
    public void CopyFrom(IReadOnlyList3D<T> source, Point3D destinationOffset) {
        ...
        CopyFrom(source, Point3D.Zero, destinationOffset, new Size3D(source.XCount, source.YCount, source.ZCount));
    }

    /// <summary>
    ///     Copies a rectangular region from the specified <see cref="List3D{T}" /> into this list.
    /// </summary>
    /// <param name="source">The source list to copy from.</param>
    ...
    public void CopyFrom(List3D<T> source, Point3D sourceOffset, Point3D destinationOffset, Size3D size) {
        ArgumentNullException.ThrowIfNull(source);
        ...
        var xReverse = ReferenceEquals(this, source) && destinationOffset.X > sourceOffset.X;
        var yReverse = ReferenceEquals(this, source) && destinationOffset.X == sourceOffset.X && destinationOffset.Y > sourceOffset.Y;
        ...
        for (var x = xStart; x != xEnd; x += xStep) {
            for (var y = yStart; y != yEnd; y += yStep) {
                var srcSpan = source._items.AsSpan(source.GetOffset(sourceOffset.X + x, sourceOffset.Y + y, sourceOffset.Z), size.Z);
                var dstSpan = _items.AsSpan(GetOffset(destinationOffset.X + x, destinationOffset.Y + y, destinationOffset.Z), size.Z);
                srcSpan.CopyTo(dstSpan);
            }
        }
    }
    ...
    /// <summary>
    ///     Copies a rectangular region from the specified three-dimensional list into this list.
    /// </summary>
    /// <param name="source">The source list to copy from.</param>
    /// <param name="sourceOffset">The starting coordinate in the source list.</param>
    /// <param name="destinationOffset">The starting coordinate in this list.</param>
    ...
    public void CopyFrom(IReadOnlyList3D<T> source, Point3D sourceOffset, Point3D destinationOffset, Size3D size) {
        ...
        if (source is List3D<T> list3D) {
            CopyFrom(list3D, sourceOffset, destinationOffset, size);
            return;
        }
        ...
        for (var x = xStart; x != xEnd; x += xStep) {
            for (var y = yStart; y != yEnd; y += yStep) {
                for (var z = 0; z < size.Z; z++) {
                    _items[GetOffset(destinationOffset.X + x, destinationOffset.Y + y, destinationOffset.Z + z)] =
                        source[sourceOffset.X + x, sourceOffset.Y + y, sourceOffset.Z + z];
                }
            }
        }
    }
