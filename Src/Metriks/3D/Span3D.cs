...
public readonly ref struct Span3D<T> {
    ...
    public Span3D<T> Slice(int x, int y, int z, int xSize, int ySize, int zSize) {
        ...
        return new Span3D<T>(_span.Slice(offset, length), xSize, ySize, zSize, StrideX, StrideY);
    }
    ...
    public Span2D<T> GetPlaneAtX(int x) {
        ...
        return new Span2D<T>(_span.Slice(offset, length), YSize, ZSize, StrideY);
    }
    ...
    /// </summary>
    public static implicit operator ReadOnlySpan3D<T>(Span3D<T> span) => new(span._span, span.XSize, span.YSize, span.ZSize, span.StrideX, span.StrideY);
