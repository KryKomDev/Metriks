...
public static class Array3D {
    ...
    public static void Copy<T>(
        ...
        if (xToCopy <= 0 || yToCopy <= 0 || zToCopy <= 0)
            return;

        var srcYSize = source.GetLength(1);
        var srcZSize = source.GetLength(2);
        var dstYSize = dest.GetLength(1);
        var dstZSize = dest.GetLength(2);
    ...
    /// <summary>
    ///     Copies a rectangular region of data from one three-dimensional array to another, using structured definitions for
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void Copy<T>(T[,,] source, T[,,] dest, Point3D offset) => Copy(source, Point3D.Zero, dest, offset, dest.Size);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void Fill<T>(T[,,] array, T item, Point3D start, Size3D size) => Fill(array, item, start.X, start.Y, start.Z, size.X, size.Y, size.Z);
    ...
    /// <param name="size">The size of the rectangular region within the array.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void Fill<T>(T[,,] array, Func<T> itemFactory, Point3D start, Size3D size) => Fill(array, itemFactory, start.X, start.Y, start.Z, size.X, size.Y, size.Z);
    ...
    public static void Clear<T>(T[,,] array, int xOffset, int yOffset, int zOffset, int xCount, int yCount, int zCount) {
        ...
        for (var x = 0; x < xCount; x++)
        for (var y = 0; y < yCount; y++) {
            var index = ((xOffset + x) * ySize + yOffset + y) * zSize + zOffset;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void Clear<T>(T[,,] array, Point3D offset, Size3D size) => Clear(array, offset.X, offset.Y, offset.Z, size.X, size.Y, size.Z);
