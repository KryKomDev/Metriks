...
public readonly ref struct ReadOnlySpan4D<T> {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ReadOnlySpan4D{T}"/> struct wrapping a flat span with given dimensions.
    /// </summary>
    ...
    public ReadOnlySpan4D(ReadOnlySpan<T> span, int wSize, int xSize, int ySize, int zSize) {
        if (wSize < 0) throw new ArgumentOutOfRangeException(nameof(wSize));
        ...
        if (wSize * xSize * ySize * zSize > span.Length) throw new ArgumentException("Span length is less than the specified dimensions.");
    ...
    public ReadOnlySpan4D(ReadOnlySpan<T> span, int wSize, int xSize, int ySize, int zSize, int strideW, int strideX, int strideY) {
        ...
        if (strideW < xSize * strideX) throw new ArgumentOutOfRangeException(nameof(strideW));
    ...
    public ReadOnlySpan4D<T> Slice(int w, int x, int y, int z, int wSize, int xSize, int ySize, int zSize) {
        ...
        return new ReadOnlySpan4D<T>(_span.Slice(offset, length), wSize, xSize, ySize, zSize, _strideW, _strideX, _strideY);
    }
    ...
    public ReadOnlySpan3D<T> GetCubeAtW(int w) {
        if ((uint)w >= (uint)_wSize) throw new ArgumentOutOfRangeException(nameof(w));
        int offset = w * _strideW;
        int length = _xSize == 0 || _ySize == 0 || _zSize == 0 
            ? 0 
            : (_xSize - 1) * _strideX + (_ySize - 1) * _strideY + _zSize;
        return new ReadOnlySpan3D<T>(_span.Slice(offset, length), _xSize, _ySize, _zSize, _strideX, _strideY);
    }
    ...
    public void CopyTo(Span4D<T> destination) {
        if (_wSize != destination.WSize || _xSize != destination.XSize || _ySize != destination.YSize || _zSize != destination.ZSize) {
