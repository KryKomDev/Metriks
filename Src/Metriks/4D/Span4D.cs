...
public readonly ref struct Span4D<T> {
    ...
    public Span4D<T> Slice(int w, int x, int y, int z, int wSize, int xSize, int ySize, int zSize) {
        ...
        return new Span4D<T>(_span.Slice(offset, length), wSize, xSize, ySize, zSize, StrideW, StrideX, StrideY);
    }
    ...
    public void CopyTo(Span4D<T> destination) {
        if (WSize != destination.WSize ||
            XSize != destination.XSize ||
            YSize != destination.YSize ||
            ZSize != destination.ZSize)
