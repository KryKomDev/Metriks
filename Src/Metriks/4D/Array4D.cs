...
public static class Array4D {
    ...
    public static void Copy<T>(
        ...
        if (wToCopy <= 0 || xToCopy <= 0 || yToCopy <= 0 || zToCopy <= 0)
            return;

        var srcXSize = source.GetLength(1);
        var srcYSize = source.GetLength(2);
        var srcZSize = source.GetLength(3);
        var dstXSize = dest.GetLength(1);
        var dstYSize = dest.GetLength(2);
        var dstZSize = dest.GetLength(3);

        ref var srcRef = ref source[0, 0, 0, 0];
        ref var dstRef = ref dest[0, 0, 0, 0];

        for (var w = 0; w < wToCopy; w++)
        for (var x = 0; x < xToCopy; x++)
        for (var y = 0; y < yToCopy; y++) {
    ...
    /// <summary>
    ///     Copies a rectangular region of data from one four-dimensional array to another, using structured definitions for
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void Copy<T>(T[,,,] source, T[,,,] dest, Point4D offset) => Copy(source, Point4D.Zero, dest, offset, dest.Size);
    ...
    public static void Fill<T>(T[,,,] array, T item, int wStart, int xStart, int yStart, int zStart, int wCount, int xCount, int yCount, int zCount) {
        if (wCount <= 0 || xCount <= 0 || yCount <= 0 || zCount <= 0)
            return;

        var xSize     = array.GetLength(1);
        var ySize     = array.GetLength(2);
        var zSize     = array.GetLength(3);
        var flatArray = MemoryMarshal.CreateSpan(ref array[0, 0, 0, 0], array.Length);

        for (var w = 0; w < wCount; w++)
        for (var x = 0; x < xCount; x++)
        for (var y = 0; y < yCount; y++) {
            var offset = (((wStart + w) * xSize + xStart + x) * ySize + yStart + y) * zSize + zStart;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void Fill<T>(T[,,,] array, T item, Point4D start, Size4D size) => Fill(array, item, start.W, start.X, start.Y, start.Z, size.W, size.X, size.Y, size.Z);
    ...
    /// <param name="size">The size of the rectangular region within the array.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void Fill<T>(T[,,,] array, Func<T> itemFactory, Point4D start, Size4D size) => Fill(array, itemFactory, start.W, start.X, start.Y, start.Z, size.W, size.X, size.Y, size.Z);
    ...
    public static void Clear<T>(T[,,,] array, int wOffset, int xOffset, int yOffset, int zOffset, int wCount, int xCount, int yCount, int zCount) {
        ...
        for (var x = 0; x < xCount; x++)
        for (var y = 0; y < yCount; y++) {
            var index = (((wOffset + w) * xSize + xOffset + x) * ySize + yOffset + y) * zSize + zOffset;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void Clear<T>(T[,,,] array, Point4D offset, Size4D size) => Clear(array, offset.W, offset.X, offset.Y, offset.Z, size.W, size.X, size.Y, size.Z);
