...
public readonly record struct Point4D : IFormattable {
    ...
    /// <returns>
    ///     A new <see cref="Point4D" /> containing the maximum W, X, Y, and Z values
    ///     from the two input points.
    /// </returns>
    public static Point4D Max(Point4D a, Point4D b) => new(Math.Max(a.W, b.W), Math.Max(a.X, b.X), Math.Max(a.Y, b.Y), Math.Max(a.Z, b.Z));
    ...
    /// <param name="b">The second <see cref="Point4D" /> to compare.</param>
    /// <returns>
    ///     A new <see cref="Point4D" /> containing the minimum W, X, Y, and Z values
    ///     from the two input points.
    /// </returns>
    public static Point4D Min(Point4D a, Point4D b) => new(Math.Min(a.W, b.W), Math.Min(a.X, b.X), Math.Min(a.Y, b.Y), Math.Min(a.Z, b.Z));
}